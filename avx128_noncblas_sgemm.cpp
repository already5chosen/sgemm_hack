...
static void avx128_noncblas_sgemm_core(
  ...
  for (m = 0; m < pPrm->M-1; A += lda*2, C += ldc*2, m += 2) {
    ...
    for (int n = 0; n < SIMD_ELEM_PEC_COL; n += COLS_PER_LOOP) {
      ...
      for (int k = 1; k < bb_nRows; k += 2) {
        ...
        acc01 = _mm_add_ps(acc01, _mm_mul_ps(a1, b));

        b = Bcol[1];
        acc10 = _mm_add_ps(acc10, _mm_mul_ps(a0, b));
        acc11 = _mm_add_ps(acc11, _mm_mul_ps(a1, b));

        b = Bcol[2];
        acc20 = _mm_add_ps(acc20, _mm_mul_ps(a0, b));
        acc21 = _mm_add_ps(acc21, _mm_mul_ps(a1, b));
      ...
      _mm_storeu_ps(&Crow0[SIMD_FACTOR*0], _mm_add_ps(_mm_mul_ps(acc00, alpha_ps), _mm_loadu_ps(&Crow0[SIMD_FACTOR*0])));
      _mm_storeu_ps(&Crow0[SIMD_FACTOR*1], _mm_add_ps(_mm_mul_ps(acc10, alpha_ps), _mm_loadu_ps(&Crow0[SIMD_FACTOR*1])));
      _mm_storeu_ps(&Crow0[SIMD_FACTOR*2], _mm_add_ps(_mm_mul_ps(acc20, alpha_ps), _mm_loadu_ps(&Crow0[SIMD_FACTOR*2])));

      _mm_storeu_ps(&Crow1[SIMD_FACTOR*0], _mm_add_ps(_mm_mul_ps(acc01, alpha_ps), _mm_loadu_ps(&Crow1[SIMD_FACTOR*0])));
      _mm_storeu_ps(&Crow1[SIMD_FACTOR*1], _mm_add_ps(_mm_mul_ps(acc11, alpha_ps), _mm_loadu_ps(&Crow1[SIMD_FACTOR*1])));
      _mm_storeu_ps(&Crow1[SIMD_FACTOR*2], _mm_add_ps(_mm_mul_ps(acc21, alpha_ps), _mm_loadu_ps(&Crow1[SIMD_FACTOR*2])));
...
static void avx128_noncblas_sgemm_core_bottomRows(
 const noncblas_sgemm_prm_t* pPrm,
 const float *A,
 float *C,
 int nRows)
...
static void avx128_noncblas_sgemm_core_rightmostColumns(
...
static void avx128_noncblas_sgemm_multC(
 int M, int N,
 float beta, 
 float *C, int ldc)
{
...
void avx128_noncblas_sgemm(
 ...
 float *C, int ldc)
{
  avx128_noncblas_sgemm_multC(M, N, beta, C, ldc);
  ...
  for (int ri = 0; ri < n_Rsteps; ++ri) {
    ...
    for (int ci = 0; ci < n_Csteps; ++ci) {
      ...
      avx128_noncblas_sgemm_core(&prm, &A[row], &C[col]);
      col += bb_nCols;
    }
    if (col < N) {
      ...
      avx128_noncblas_sgemm_core_rightmostColumns(&prm, &A[row], &C[col], N-col, bb_nRows);
    }
    row += bb_nRows;
  }
  if (row < K) {
    ...
    for (int ci = 0; ci < n_Csteps; ++ci) {
      ...
      avx128_noncblas_sgemm_core_bottomRows(&prm, &A[row], &C[col], K-row);
      col += bb_nCols;
    }
    if (col < N) {
      ...
      avx128_noncblas_sgemm_core_rightmostColumns(&prm, &A[row], &C[col], N-col, K-row);
    }
  }
}
