...
static void fma256_noncblas_sgemm_core(noncblas_sgemm_prm_t* pPrm, const float  *A, float *C)
{
  ...
  for (m = 0; m < pPrm->M - A_WORDS_PER_ITER + 1;
    ...
    _mm256_storeu_ps(&Ccol[SIMD_FACTOR * 3], _mm256_fmadd_ps(acc32, alpha_ps, _mm256_loadu_ps(&Ccol[SIMD_FACTOR * 3])));
  }

  // process up to 2 bottom rows of A, 1 row at time
 ...
 int                   nRows) // nRows <= bb_nRows
{
  ...
  for (m = 0; m < pPrm->M - A_WORDS_PER_ITER + 1;
    ...
    pPrm->cc[2 * B_WORDS_PER_ITER + 3] = _mm256_fmadd_ps(acc32, alpha_ps, pPrm->cc[2 * B_WORDS_PER_ITER + 3]);
