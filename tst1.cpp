...
int main(int argz, char** argv)
{
  ...
  for (int arg_i = 1; arg_i < argz; ++arg_i) {
    ...
    for (int pref_i = 0; pref_i < prefTabLen; ++pref_i) {
      ...
      if ( _strnicmp(pref, arg, preflen)==0 && arg[preflen]=='=') {
        if (pref_i < 2) {
  ...
  printf("Testing my 128-bit AVX hack (5x2 inner loop)...\n");
  test_noncblas_sgemm(M, N, K, alpha
    , &A.at(0), lda, &B.at(0), ldb, beta, &C.at(0), ldc
    , nIter, &srcC.at(0),
  ...
  printf("Testing my 256-bit AVX hack (5x2 inner loop)...\n");
  test_noncblas_sgemm(M, N, K, alpha
    , &A.at(0), lda, &B.at(0), ldb, beta, &C.at(0), ldc
    , nIter, &srcC.at(0),
 ...
 )
{
  ...
  printf(": med %.0f. %.3f FLOP/clk\n", double(dt[nIter/2]), double(M)*N*K*2/double(dt[nIter/2]));
